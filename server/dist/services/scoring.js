...
exports.scoreGroupRoundBonuses = scoreGroupRoundBonuses;
exports.takeLeaderboardSnapshot = takeLeaderboardSnapshot;
exports.scoreSpecialBets = scoreSpecialBets;
...
async function scoreMatch(matchId) {
    ...
    await prisma_1.prisma.bonusLog.deleteMany({ where: { reason: `UNIQUE_EXACT_${matchId}` } });
    // Unique exact bonus: if exactly one person predicted the right score they get +1
    const exactBetters = allBets.filter((b) => b.predictedHome === match.homeScore && b.predictedAway === match.awayScore);
    ...
    for (const bet of allBets) {
        ...
        if (isExact)
            ...
            points = pts.winner;
        await prisma_1.prisma.matchBet.update({
            where: { id: bet.id },
    ...
    if (uniqueExact) {
        await prisma_1.prisma.bonusLog.create({
            data: {
...
function exactScoreLadder(exactCount) {
    if (exactCount >= 24)
        return 15;
    if (exactCount >= 23)
        return 12;
...
function accuracyLadder(correctCount) {
    if (correctCount >= 24)
        return 10;
    if (correctCount >= 23)
        return 8;
...
async function scoreGroupRoundBonuses(groupRound) {
    const roundMatches = await prisma_1.prisma.match.findMany({
        where: { stage: 'GROUP', groupRound },
    });
    if (roundMatches.length !== 24)
        return; // round not fully seeded
    ...
    for (const user of users) {
        ...
        await prisma_1.prisma.bonusLog.deleteMany({
            where: { userId: user.id, reason: { startsWith: `R${groupRound}_` } },
        });
        const userBets = await prisma_1.prisma.matchBet.findMany({
...
async function takeLeaderboardSnapshot(label) {
    const { entries } = await getLeaderboard();
    for (const entry of entries) {
        await prisma_1.prisma.rankSnapshot.upsert({
...
// ─── Score special bets ───────────────────────────────────────────────────────
async function scoreSpecialBets(type, winnerTeamId, winnerPlayerName) {
    const bets = await prisma_1.prisma.specialBet.findMany({ where: { type } });
    ...
    for (const bet of bets) {
        ...
        await prisma_1.prisma.specialBet.update({
            where: { id: bet.id },
...
// ─── Leaderboard ──────────────────────────────────────────────────────────────
async function getLeaderboard(requestingUserId) {
    ...
    const provisionalUniqueExactMap = new Map();
    if (hasLiveGames) {
        for (const match of liveMatches) {
            ...
            for (const bet of match.bets) {
                ...
                if (isExact)
                    ...
                    p = pts.winner;
                if (p > 0)
                    provisionalMap.set(bet.userId, (provisionalMap.get(bet.userId) ?? 0) + p);
            }
            // Provisional unique-exact: if exactly one bettor has the current live score → provisional +1
            const exactBetters = match.bets.filter(b => b.predictedHome === match.homeScore && b.predictedAway === match.awayScore);
    ...
    const allGroupMatches = await prisma_1.prisma.match.findMany({
    ...
    const inProgressRoundMatchIds = new Map(); // round → finished matchId set
    const matchScoreMap = new Map(); // matchId → scores
    for (const [round, matches] of roundMap) {
        if (matches.length !== 24)
            continue; // round not fully seeded yet
    ...
    const entries = users.map((user) => {
        const matchPoints = user.matchBets.reduce((s, b) => s + (b.pointsAwarded ?? 0), 0);
        const specialPoints = user.specialBets.reduce((s, b) => s + (b.pointsAwarded ?? 0), 0);
        const bonusPoints = user.bonusLogs.reduce((s, b) => s + b.points, 0);
        ...
        if (hasInProgressRound) {
            for (const [round, finishedIds] of inProgressRoundMatchIds) {
                ...
                const roundBets = user.matchBets.filter(b => b.match.stage === 'GROUP' &&
                ...
                for (const bet of roundBets) {
                    ...
                    const correctWinner = getWinner(bet.predictedHome, bet.predictedAway) ===
                ...
                provisionalBonusPoints += Math.max(exactScoreLadder(exactCount), accuracyLadder(correctCount));
            }
        }
        let exactScores = 0;
        let correctScores = 0;
        ...
        const sbChampion = user.specialBets.find((b) => b.type === 'CHAMPION');
        const sbScorer = user.specialBets.find((b) => b.type === 'TOP_SCORER');
        const sbAssists = user.specialBets.find((b) => b.type === 'TOP_ASSISTS');
    ...
    entries.sort((a, b) => (b.totalPoints + b.provisionalPoints + b.provisionalBonusPoints) -
    ...
    const ranked = entries.map((e, i) => {
        ...
        if (i > 0 && prevLiveTotal !== liveTotal)
            rank = i + 1;
        return { rank, ...e };
    });
