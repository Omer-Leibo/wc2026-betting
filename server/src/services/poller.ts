...
export async function startPoller(): Promise<void> {
  ...
  const intervalMs = parseInt(process.env.SYNC_INTERVAL_MS ?? String(DEFAULT_INTERVAL_MS));
  ...
  // Start recurring live poll
  pollerHandle = setInterval(async () => {
    if (!isTournamentActive()) {
      console.log('[Poller] Tournament not active — skipping live poll');
      return;
    }
  ...
  console.log(`[Poller] Auto-sync started — polling every ${intervalMs / 1000}s`);
}
